/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import TariffGroupDetailComponent from '@/entities/tariff-group/tariff-group-details.vue';
import TariffGroupClass from '@/entities/tariff-group/tariff-group-details.component';
import TariffGroupService from '@/entities/tariff-group/tariff-group.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('TariffGroup Management Detail Component', () => {
    let wrapper: Wrapper<TariffGroupClass>;
    let comp: TariffGroupClass;
    let tariffGroupServiceStub: SinonStubbedInstance<TariffGroupService>;

    beforeEach(() => {
      tariffGroupServiceStub = sinon.createStubInstance<TariffGroupService>(TariffGroupService);

      wrapper = shallowMount<TariffGroupClass>(TariffGroupDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { tariffGroupService: () => tariffGroupServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundTariffGroup = { id: 123 };
        tariffGroupServiceStub.find.resolves(foundTariffGroup);

        // WHEN
        comp.retrieveTariffGroup(123);
        await comp.$nextTick();

        // THEN
        expect(comp.tariffGroup).toBe(foundTariffGroup);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundTariffGroup = { id: 123 };
        tariffGroupServiceStub.find.resolves(foundTariffGroup);

        // WHEN
        comp.beforeRouteEnter({ params: { tariffGroupId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.tariffGroup).toBe(foundTariffGroup);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
