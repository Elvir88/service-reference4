/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import Router from 'vue-router';
import { ToastPlugin } from 'bootstrap-vue';

import * as config from '@/shared/config/config';
import HouseUpdateComponent from '@/entities/house/house-update.vue';
import HouseClass from '@/entities/house/house-update.component';
import HouseService from '@/entities/house/house.service';

import LocationService from '@/entities/location/location.service';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
const router = new Router();
localVue.use(Router);
localVue.use(ToastPlugin);
localVue.component('font-awesome-icon', {});
localVue.component('b-input-group', {});
localVue.component('b-input-group-prepend', {});
localVue.component('b-form-datepicker', {});
localVue.component('b-form-input', {});

describe('Component Tests', () => {
  describe('House Management Update Component', () => {
    let wrapper: Wrapper<HouseClass>;
    let comp: HouseClass;
    let houseServiceStub: SinonStubbedInstance<HouseService>;

    beforeEach(() => {
      houseServiceStub = sinon.createStubInstance<HouseService>(HouseService);

      wrapper = shallowMount<HouseClass>(HouseUpdateComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: {
          houseService: () => houseServiceStub,
          alertService: () => new AlertService(),

          locationService: () =>
            sinon.createStubInstance<LocationService>(LocationService, {
              retrieve: sinon.stub().resolves({}),
            } as any),
        },
      });
      comp = wrapper.vm;
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const entity = { id: 123 };
        comp.house = entity;
        houseServiceStub.update.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(houseServiceStub.update.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        comp.house = entity;
        houseServiceStub.create.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(houseServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundHouse = { id: 123 };
        houseServiceStub.find.resolves(foundHouse);
        houseServiceStub.retrieve.resolves([foundHouse]);

        // WHEN
        comp.beforeRouteEnter({ params: { houseId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.house).toBe(foundHouse);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
